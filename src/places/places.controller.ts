...
export class PlacesController {
  ...
  async getRecommended() {
    ...
    return places.map(p => new PlaceDto({ ...p, categories: p.placeCategories?.map(pc => pc.category.name) || [], imageUrls: p.images?.map(i => i.url) || [] }));
  }
  ...
  async findAll() {
    ...
    return places.map(
      (place) =>
        new PlaceDto({
          ...place,
          categories: place.placeCategories?.map((pc) => pc.category.name) || [],
